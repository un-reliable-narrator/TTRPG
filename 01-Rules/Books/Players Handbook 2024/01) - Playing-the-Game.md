...
- **Jared.** Sure!  
- **Phillip (rolling a `dice: d20`).** Ugh. Seven.  
...
The dice used in D&D are referred to by the letter *d* followed by the die's number of sides: `dice: d4`, `dice: d6`, `dice: d8`, `dice: d10`, `dice: d12`, and `dice: d20`. For instance, a `dice: d6` is a six-sided die (the cube that many games use). The illustration on this page shows what each die looks like.
...
The same *d* notation appears in the expression "`dice: 1d3`," even though it's rare to find dice with only three sides. To simulate rolling `dice: 1d3`, roll `dice: 1d6` and divide the number rolled by 2 (round up).
...
The 20-sided die (`dice: d20`) is the most important die you'll use in the game. It's central to the core mechanic—called [D20 Tests](d20-test.md)—the game uses to determine whether creatures succeed or fail at the things they attempt during the game (see ""D20 Tests"" later in this chapter). You roll a `dice: d20` whenever your character tries to do something that the DM decides has a chance of both success and failure. The higher your roll, the more likely it is that you succeed.

#### Damage

The most common use for dice other than the `dice: d20` is to determine damage. When you make a successful attack roll (one kind of [D20 Test](d20-test.md)), you roll damage to see how effective the attack is. When you cast a spell, you might also roll damage. Different weapons and spells use different dice for damage. For example, a [dagger](dagger.md) uses `dice: 1d4`, a [Greataxe](Greataxe.md) uses `dice: 1d12`, and a [Fireball](Fireball.md) spell uses `8d6`.

#### Random Tables

Occasionally, you'll see a table that uses a die roll to generate a random result. On these tables, you'll see a die expression, such as `dice: d10` or `d100`, in the header of the leftmost column. Roll that die, and find the number you rolled (or a range containing that number) in that column. Read across that row for the result. For example, the "Trinkets table" in "chapter 2" uses a `d100`.
...
> 
> Part of the fun of D&D is interpreting what die rolls mean in the story. The randomness that dice bring often introduces the unexpected: an expert fails, a novice succeeds, or any number of other twists. When a [D20 Test](d20-test.md) or another roll produces a surprise, the Dungeon Master often gives a fun description of what happened. If you're a player, look for opportunities to interpret your character's die rolls and describe why things unfolded unexpectedly or even humorously.
...
![The Six Abilities; Ability Descriptions](the-six-abilities-ability-descriptions.md)
...
![Ability Scores](ability-scores.md)

### Ability Modifiers

Each ability has a modifier that you apply whenever you make a [D20 Test](d20-test.md) with that ability (explained in ""D20 Tests""). An ability modifier is derived from its score, as shown in the Ability Modifiers table.

![Ability Modifiers](ability-modifiers.md)

## D20 Tests

When the outcome of an action is uncertain, the game uses a `dice: d20` roll to determine success or failure. These rolls are called [D20 Tests](d20-test.md), and they come in three kinds: "ability checks", "saving throws", and "attack rolls". They follow these steps:

- **Roll 1d20.** You always want to roll high. If the roll has [[Advantage|Advantage]] or [[01-Rules/Rules/Disadvantage|Disadvantage]] (described "later in this chapter"), you roll two d20s, but you use the number from only one of them—the higher one if you have Advantage or the lower one if you have Disadvantage.  
- **Add Modifiers.** Add these modifiers to the number rolled on the `dice: d20`:  
-     - **The Relevant Ability Modifier.** This chapter and the "rules glossary" explain which ability modifiers to use for various [D20 Tests](d20-test).    
        - **Your Proficiency Bonus If Relevant.** Each creature has a Proficiency Bonus, a number added when making a [D20 Test](d20-test) that uses something, such as a skill, in which the creature has proficiency. See ""Proficiency"" later in this chapter.    
...
- **Compare the Total to a Target Number.** If the total of the `dice: d20` and its modifiers equals or exceeds the target number, the [D20 Test](d20-test.md) succeeds. Otherwise, it fails. The Dungeon Master determines target numbers and tells players whether their rolls are successful. The target number for an ability check or a saving throw is called a [Difficulty Class](difficulty-class.md) (DC). The target number for an attack roll is called an [Armor Class](Armor%20Class.md) (AC), which appears on a character sheet or in a [stat block](stat-block.md).  
...
![Ability Modifier; Ability Check Examples](ability-modifier-ability-check-examples.md)

#### Proficiency Bonus

Add your Proficiency Bonus to an ability check when the DM determines that a skill or tool proficiency is relevant to the check and you have that proficiency. For example, if a rule refers to a Strength ([Acrobatics](Acrobatics.md) or [Athletics](Athletics.md)) check, you can add your Proficiency Bonus to the check if you have proficiency in the [Acrobatics](Acrobatics.md) or [Athletics](Athletics.md) skill. See ""Proficiency"" later in this chapter for more information about skill and tool proficiencies.
...
![Typical Difficulty Classes](typical-difficulty-classes.md)
...
![Ability Modifier; Saving Throw Examples](ability-modifier-saving-throw-examples.md)
...
![Ability Modifier; Attack Roll Abilities](ability-modifier-attack-roll-abilities.md)
...
If you roll a 20 on the `dice: d20` (called a "natural 20") for an attack roll, the attack hits regardless of any modifiers or the target's AC. This is called a Critical Hit (see ""Combat"" later in this chapter).

If you roll a 1 on the `dice: d20` (a "natural 1") for an attack roll, the attack misses regardless of any modifiers or the target's AC.

### Advantage/Disadvantage


Sometimes a [D20 Test](d20-test.md) is modified by Advantage or Disadvantage. Advantage reflects the positive circumstances surrounding a `dice: d20` roll, while Disadvantage reflects negative circumstances.
...
When a roll has either Advantage or Disadvantage, roll a second `dice: d20` when you make the roll. Use the higher of the two rolls if you have Advantage, and use the lower roll if you have Disadvantage. For example, if you have Disadvantage and roll an 18 and a 3, use the 3. If you instead have Advantage and roll those numbers, use the 18.
...
If circumstances cause a roll to have both Advantage and Disadvantage, the roll has neither of them, and you roll one `dice: d20`. This is true even if multiple circumstances impose Disadvantage and only one grants Advantage or vice versa. In such a situation, you have neither Advantage nor Disadvantage.

#### Interactions with Rerolls

When you have Advantage or Disadvantage and something in the game lets you reroll or replace the `dice: d20`, you can reroll or replace only one die, not both. You choose which one.

For example, if you have [Heroic Inspiration](Heroic-Inspiration.md) (see the sidebar) and roll a 3 and an 18 on an ability check that has Advantage or Disadvantage, you could expend your Heroic Inspiration to reroll one of those dice, not both of them.
...
Characters and monsters are good at various things. Some are skilled with many weapons, while others can use only a few. Some are better at understanding people's motives, and others are better at unlocking the secrets of the multiverse. All creatures have a Proficiency Bonus, which reflects the impact that training has on the creature's capabilities. A character's Proficiency Bonus increases as the character gains levels (described in "chapter 2"). A monster's Proficiency Bonus is based on its [Challenge Rating](challenge-rating.md). The Proficiency Bonus table shows how the bonus is determined.

This bonus is applied to a [D20 Test](d20-test.md) when the creature has proficiency in a skill, in a saving throw, or with an item that the creature uses to make the [D20 Test](d20-test.md). The bonus is also used for spell attacks and for calculating the DC of saving throws for spells.

![Proficiency Bonus](Proficiency-Bonus.md)

### The Bonus Doesn't Stack

Your Proficiency Bonus can't be added to a die roll or another number more than once. For example, if a rule allows you to make a Charisma ([Deception](Deception.md) or [[Persuasion]]) check, you add your Proficiency Bonus if you're proficient in either skill, but you don't add it twice if you're proficient in both skills.

Occasionally, a Proficiency Bonus might be multiplied or divided (doubled or halved, for example) before being added. For example, the [Expertise](01-Rules/Rules/Expertise.md) feature doubles the Proficiency Bonus for certain ability checks. Whenever the bonus is used, it can be multiplied only once and divided only once.
...
If a creature is proficient in a skill, the creature applies its Proficiency Bonus to ability checks involving that skill. Without proficiency in a skill, a creature can still make ability checks involving that skill but doesn't add its Proficiency Bonus. For example, if a character tries to climb a cliff, the DM might ask for a Strength ([Athletics](Athletics.md)) check. If the character has [Athletics](Athletics.md) proficiency, the character adds their Proficiency Bonus to the Strength check. If the character lacks that proficiency, they make the check without adding their Proficiency Bonus.
...
![Skill List; Skills](skill-list Skills)
...
![Actions](01-Rules/Actions/Actions.md)

Player characters and monsters can also do things not covered by these actions. Many class features and other abilities provide additional action options, and you can improvise other actions. When you describe an action not detailed elsewhere in the rules, the Dungeon Master tells you whether that action is possible and what kind of [D20 Test](d20-test.md) you need to make, if any.
...
Actions can come up in other situations, too: in a social interaction, you can try to [Influence](Influence.md) a creature or use the [Search](Search.md) action to read the creature's body language, but you can't do both at the same time. And when you're exploring a dungeon, you can't simultaneously use the [Search](Search.md) action to look for traps and use the [Help](Help.md) action to aid another character who's trying to open a stuck door (with the [Utilize](Utilize.md) action).
...
Certain special abilities, spells, and situations allow you to take a special action called a Reaction. A Reaction is an instant response to a trigger of some kind, which can occur on your turn or on someone else's. The [Opportunity Attack](Opportunity%20Attacks.md), described "later in this chapter", is the most common type of Reaction.
...
An NPC's attitude toward your character is [Friendly](friendly-attitude.md), [Indifferent](indifferent-attitude.md), or [Hostile](hostile-attitude.md), as defined in the "rules glossary". Friendly NPCs are predisposed to help, and Hostile ones are inclined to hinder.
...
Ability checks can be key in determining the outcome of a social interaction. Your roleplaying efforts can alter an NPC's attitude, but there might still be an element of chance if the DM wants dice to play a role in determining an NPC's response to you. In such situations, the DM will typically ask you to take the [Influence](Influence.md) action.

Pay attention to your skill proficiencies when thinking of how you will interact with an NPC; use an approach that relies on your group's skill proficiencies. For example, if the group needs to trick a guard into letting them into a castle, the Rogue who is proficient in [Deception](Deception.md) should lead the discussion.
...
> 
> Phillip's character, Gareth, makes a Wisdom ([Insight](Insight.md)) check to determine whether he reads Ismark's unspoken cues. Wisdom is the ability that measures perceptiveness and intuition. Insight reflects a character's skill at reading other people's moods and intentions. The DM set the DC at 15, which Gareth beats. With a successful Wisdom ([Insight](Insight.md)) check, Gareth gleans information that wasn't obvious, so it wasn't part of the DM's earlier narration. For more information, see ""Ability Scores"" and ""Proficiency"" earlier in this chapter and the [Search](Search.md) action in the "rules glossary".
^1-ability-checks
...
> 
> Gareth takes the [Influence](Influence.md) action, which characters can take to influence an NPC. It's easier to influence an NPC who's already disposed to help you, as Ismark is here. Ismark's [Friendly](friendly-attitude.md) attitude gives Phillip Advantage on the roll. In this case, [[Advantage|Advantage]] made the difference between success and failure.
^2-influencing-npcs
...
An area might be Lightly or Heavily Obscured. In a Lightly Obscured area—such as an area with Dim Light, patchy fog, or moderate foliage—you have [[01-Rules/Rules/Disadvantage|Disadvantage]] on Wisdom ([Perception](Perception.md)) checks that rely on sight.
...
Dim Light, also called shadows, creates a [Lightly Obscured](lightly-obscured.md) area. An area of Dim Light is usually a boundary between Bright Light and surrounding Darkness. The soft light of twilight and dawn also counts as Dim Light. A full moon might bathe the land in Dim Light.

##### Darkness

Darkness creates a [Heavily Obscured](heavily-obscured.md) area. Characters face Darkness outdoors at night (even most moonlit nights), within the confines of an unlit dungeon, or in an area of magical Darkness.
...
Adventurers and monsters often hide, whether to spy on one another, sneak past a guardian, or set an ambush. The Dungeon Master decides when circumstances are appropriate for hiding. When you try to hide, you take the [Hide](Hide.md) action.
...
When time is short, such as in combat, interactions with objects are limited: one free interaction per turn. That interaction must occur during a creature's movement or action. Any additional interactions require the [Utilize](Utilize.md) action, as explained in ""Combat"" later in this chapter.

#### Finding Hidden Objects

When your character searches for hidden things, such as a secret door or a trap, the DM typically asks you to make a Wisdom ([Perception](Perception.md)) check, provided you describe the character searching in the hidden object's vicinity. On a success, you find the object, other important details, or both.

If you describe your character searching nowhere near a hidden object, a Wisdom ([Perception](Perception.md)) check won't reveal the object, no matter the check's total.

#### Carrying Objects

You can usually carry your gear and treasure without worrying about the weight of those objects. If you try to haul an unusually heavy object or a massive number of lighter objects, the DM might require you to abide by the rules for [carrying capacity](carrying-capacity.md) in the "rules glossary".

#### Breaking Objects

As an action, you can automatically break or otherwise destroy a fragile, nonmagical object, such as a glass container or a piece of paper. If you try to damage something more resilient, the DM might use the rules on [breaking objects](breaking-objects.md) in the "rules glossary".

### Hazards

Monsters are the main perils characters face, but other dangers await. The rules glossary defines the following hazards:

[Burning](01-Rules/Hazards/Burning.md)

[Dehydration](01-Rules/Hazards/Dehydration.md)

[Falling](01-Rules/Hazards/Falling.md)

[Malnutrition](01-Rules/Hazards/Malnutrition.md)

[Suffocation](01-Rules/Hazards/Suffocation.md)
...
While traveling outside combat, a group can move at a Fast, Normal, or Slow pace, as shown on the Travel Pace table. The table states how far the party can move in a period of time; if riding horses or other mounts, the group can move twice that distance for 1 hour, after which the mounts need a [Short](short-rest.md) or [Long Rest](long-rest.md) before they can move at that increased pace again (see "chapter 6" for a selection of mounts for sale). The *Dungeon Master's Guide* has rules that affect which pace you can choose in certain types of terrain.
...
Traveling at a Fast pace imposes [[01-Rules/Rules/Disadvantage|Disadvantage]] on a traveler's Wisdom ([Perception](Perception.md)or [[Survival]]) and Dexterity ([Stealth](Stealth.md)) checks.

##### Normal

Traveling at a Normal pace imposes [[01-Rules/Rules/Disadvantage|Disadvantage]] on Dexterity ([Stealth](Stealth.md)) checks.

##### Slow

Traveling at a Slow pace grants [[Advantage|Advantage]] on Wisdom ([Perception](Perception.md)or [[Survival]]) checks.
...
> 
> You usually have a chance of success on an ability check even if you don't have proficiency in a relevant skill. In the example, Phillip gets a 20 without proficiency. The `dice: d20` brings many surprises to the game—surprises that the DM then interprets.
...
> 
> Adventure environments are full of secrets: traps, concealed doors, loose floorboards, and hidden treasures. Characters often use Wisdom checks to find things—see the [Search](Search.md) action in the "rules glossary"—but sometimes they stumble on them accidentally, as Auro did here.
...
A typical combat encounter is a clash between two sides: a flurry of weapon swings, feints, parries, footwork, and spellcasting. The game organizes combat into a cycle of rounds and turns. A round represents about 6 seconds in the game world. During a round, each participant in a battle takes a turn. The order of turns is determined at the beginning of combat when everyone rolls [Initiative](01-Rules/Rules/Initiative.md). Once everyone has taken a turn, the fight continues to the next round if neither side is defeated.
...
- **Roll Initiative.** Everyone involved in the combat encounter rolls [Initiative](01-Rules/Rules/Initiative.md), determining the order of combatants' turns.  
...
If a combatant is surprised by combat starting, that combatant has [[01-Rules/Rules/Disadvantage|Disadvantage]] on their Initiative roll. For example, if an ambusher starts combat while hidden from a foe who is unaware that combat is starting, that foe is surprised.
...
On your turn, you can move a distance up to your [Speed](01-Rules/Rules/Speed.md) and take one action. You decide whether to move first or take your action first.
...
If you want to interact with a second object, you need to take the [Utilize](Utilize.md) action. Some magic items and other special objects always require an action to use, as stated in their descriptions.

The DM might require you to use an action for any of these activities when it needs special care or when it presents an unusual obstacle. For instance, the DM might require you to take the [Utilize](Utilize.md) action to open a stuck door or turn a crank to lower a drawbridge.

##### Doing Nothing on Your Turn

You can forgo moving, taking an action, or doing anything at all on your turn. If you can't decide what to do, consider taking the defensive [Dodge](Dodge.md) action or the [Ready](Ready.md) action to delay acting.
...
On your turn, you can move a distance equal to your [Speed](01-Rules/Rules/Speed.md) or less. Or you can decide not to move.

Your movement can include [Climbing](01-Rules/Rules/Climbing.md), [Crawling](01-Rules/Rules/Crawling.md), [Jumping](01-Rules/Rules/Jumping.md), and [Swimming](01-Rules/Rules/Swimming.md) (each explained in the "rules glossary"). These different modes of movement can be combined with your regular movement, or they can constitute your entire move.
...
A character's Speed is determined during character creation. A monster's Speed is noted in the monster's stat block. See the "rules glossary" for more about Speed as well as about special speeds, such as a [Climb Speed](climb-speed.md), [Fly Speed](fly-speed.md), or [Swim Speed](swim-speed.md).
...
![Creature Size and Space](creature-size-and-space.md)
...
Another creature's space is [Difficult Terrain](difficult-terrain.md) for you unless that creature is Tiny or your ally.
...
- **Determine Modifiers.** The DM determines whether the target has [Cover]%20Backlink%20Glossary/Rules/rules/variant-rules/cover) (see the next section) and whether you have [[Advantage|Advantage]] or [[01-Rules/Rules/Disadvantage|Disadvantage]] against the target. In addition, spells, special abilities, and other effects can apply penalties or bonuses to your attack roll.  
...
A target can benefit from cover only when an attack or other effect originates on the opposite side of the cover. If a target is behind multiple sources of cover, only the most protective degree of cover applies; the degrees aren't added together. For example, if a target is behind a creature that gives [[Half Cover]] and a tree trunk that gives [Three-Quarters Cover](03)%20Resources/Tables/RAW/Combat/Cover.md), the target has Three-Quarters Cover.

![Cover](01-Rules/Rules/variant-rules/cover.md)

> [!note] Unseen Attackers and Targets
> 
> When you make an attack roll against a target you can't see, you have [[01-Rules/Rules/Disadvantage|Disadvantage]] on the roll. This is true whether you're guessing the target's location or targeting a creature you can hear but not see. If the target isn't in the location you targeted, you miss.
> 
> When a creature can't see you, you have [[Advantage|Advantage]] on attack rolls against it.
> 
...
Some ranged attacks, such as those made with a [Longbow](Longbow.md), have two ranges. The smaller number is the normal range, and the larger number is the long range. Your attack roll has [[01-Rules/Rules/Disadvantage|Disadvantage]] when your target is beyond normal range, and you can't attack a target beyond long range.

#### Ranged Attacks in Close Combat

Aiming a ranged attack is more difficult when a foe is next to you. When you make a ranged attack roll with a weapon, a spell, or some other means, you have [[01-Rules/Rules/Disadvantage|Disadvantage]] on the roll if you are within 5 feet of an enemy who can see you and doesn't have the [Incapacitated](Conditions.md#Incapacitated) condition.

### Melee Attacks

A melee attack allows you to attack a target within your reach. A melee attack typically uses a handheld weapon or an [Unarmed Strike](Unarmed%20Strike.md). Many monsters make melee attacks with claws, teeth, or other body parts. A few spells also involve melee attacks.
...
You can avoid provoking an Opportunity Attack by taking the [Disengage](Disengage.md) action. You also don't provoke an Opportunity Attack when you [Teleport](Teleportation.md) or when you are moved without using your movement, action, Bonus Action, or Reaction. For example, you don't provoke an Opportunity Attack if an explosion hurls you out of a foe's reach or if you fall past an enemy.

##### Making an Opportunity Attack

You can make an Opportunity Attack when a creature that you can see leaves your reach. To make the attack, take a Reaction to make one melee attack with a weapon or an [Unarmed Strike](Unarmed%20Strike.md) against that creature. The attack occurs right before it leaves your reach.
...
During your move, you can mount a creature that is within 5 feet of you or dismount. Doing so costs an amount of movement equal to half your [Speed](01-Rules/Rules/Speed.md) (round down). For example, if your Speed is 30 feet, you spend 15 feet of movement to mount a horse.
...
The [Initiative](01-Rules/Rules/Initiative.md) of a controlled mount changes to match yours when you mount it. It moves on your turn as you direct it, and it has only three action options during that turn: [Dash](Dash.md), [Disengage](Disengage.md), and [Dodge](Dodge.md). A controlled mount can move and act even on the turn that you mount it.
...
When making a melee attack roll with a weapon underwater, a creature that lacks a [Swim Speed](swim-speed.md) has [[01-Rules/Rules/Disadvantage|Disadvantage]] on the attack roll unless the weapon deals Piercing damage.
...
> [!note] Resting
> 
> Adventurers can't spend every hour adventuring. They need rest. Any creature can take hour-long [Short Rests](short-rest.md) in the midst of a day and an 8-hour [Long Rest](long-rest.md) to end it. Regaining Hit Points is one of the main benefits of a rest. The "rules glossary" provides the rules for Short and Long Rests.
^resting
...
When attacking with a weapon, you add your ability modifier—the same modifier used for the attack roll—to the damage roll. A spell tells you which dice to roll for damage and whether to add any modifiers. Unless a rule says otherwise, you don't add your ability modifier to a fixed damage amount that doesn't use a roll, such as the damage of a [Blowgun](Blowgun.md). See "chapter 6" for weapons' damage dice and "chapter 7" for spells' damage dice.

### Critical Hits

When you score a Critical Hit, you deal extra damage. Roll the attack's damage dice twice, add them together, and add any relevant modifiers as normal. For example, if you score a Critical Hit with a [dagger](dagger.md), roll `dice: 2d4` for the damage rather than `dice: 1d4`, and add your relevant ability modifier. If the attack involves other damage dice, such as from the Rogue's Sneak Attack feature, you also roll those dice twice.
...
When you create a damaging effect that forces two or more targets to make saving throws against it at the same time, roll the damage once for all the targets. For example, when a wizard casts [Fireball](Fireball.md), the spell's damage is rolled once for all creatures caught in the blast.
...
Each instance of damage has a type, like Fire or Slashing. Damage types are listed in the "rules glossary" and have no rules of their own, but other rules, such as [Resistance](01-Rules/Rules/Resistance.md), rely on damage types.
...
Hit Points can be restored by magic, such as the [Cure Wounds](Cure-Wounds.md) spell or a [Potion of Healing](potion-of-healing.md), or by a [Short](short-rest.md) or [Long Rest](long-rest.md).
...
If your character dies, others might find a magical way to revive your character, such as with the [Raise Dead](Raise-Dead.md) spell. Or talk with the DM about making a new character to join the group. The "rules glossary" has more information on being [Dead](01-Rules/Rules/Dead.md).

#### Falling Unconscious

If you reach 0 Hit Points and don't die instantly, you have the [Unconscious](Conditions.md#Unconscious) condition until you regain any Hit Points, and you now face making [Death Saving Throws](death-saving-throw.md) (see below).

> [!note] Knocking Out a Creature
> 
> When you would reduce a creature to 0 Hit Points with a melee attack, you can instead reduce the creature to 1 Hit Point and give it the [Unconscious](Conditions.md#Unconscious) condition. It then starts a [Short Rest](short-rest.md), at the end of which that condition ends on it. The condition ends early if the creature regains any Hit Points or if someone takes an action to administer first aid to it, making a successful DC 10 Wisdom ([Medicine](Medicine.md)) check.
...
Roll `dice: 1d20`. If the roll is 10 or higher, you succeed. Otherwise, you fail. A success or failure has no effect by itself. On your third success, you become [Stable](01-Rules/Rules/Stable.md) (see "Stabilizing a Character" below). On your third failure, you die.
...
When you roll a 1 on the `dice: d20` for a Death Saving Throw, you suffer two failures. If you roll a 20 on the `dice: d20`, you regain 1 Hit Point.
...
You can take the [Help](Help.md) action to try to stabilize a creature with 0 Hit Points, which requires a successful DC 10 Wisdom ([Medicine](Medicine.md)) check.

A Stable creature doesn't make [Death Saving Throws](death-saving-throw.md) even though it has 0 Hit Points, but it still has the [Unconscious](Conditions.md#Unconscious) condition. If the creature takes damage, it stops being Stable and starts making [Death Saving Throws](death-saving-throw.md) again. A Stable creature that isn't healed regains 1 Hit Point after `dice: 1d4` hours.
...
Temporary Hit Points last until they're depleted or you finish a [Long Rest](long-rest.md).
...
> 
> The next step in combat is for everyone involved to roll [Initiative](01-Rules/Rules/Initiative.md).
...
> 
> Mirabella casts the [Shield](01-Rules/Spells/Core/Level%201/Shield.md) spell. It's a special property of this spell that it can be cast even when it's not the caster's turn, using her Reaction (see ""Actions"" earlier in this chapter).
^12-spells-and-reactions

> [!note] (13)—Special Abilities
> 
> Auro's Sneak Attack lets him deal extra damage to the skeletons, and his Cunning Action lets him [Disengage](Disengage.md) as a Bonus Action. These are examples of class features (described in "chapter 3") that let characters bend the rules.
...
> 
> Spells and many other effects in the game fill a specific area that is defined by the rules, like the 15-foot [Cube](cube-area-of-effect.md) of the [Thunderwave](Thunderwave.md) spell. See the "rules glossary" for more on areas of effect.
^15-areas-of-effect

> [!note] (16)—Saving Throws
> 
> The skeletons make Constitution saving throws to resist the [Thunderwave](Thunderwave.md) spell. When they fail those saves, they take damage and are pushed, as defined in that spell's description.
^16-saving-throws
...
> 
> Gareth's Sear Undead is a class feature (see "chapter 3"). As with Mirabella's [Thunderwave](Thunderwave.md) spell, Sear Undead forces the skeletons to make a saving throw.
^17-saving-throws
