...
In addition to its aforementioned properties, each Moonblade has a minor property determined by rolling on the [Magic](Magic.md) Item's Minor Property table.
...
| 61-75 | When you hit with an attack roll using the *Moonblade*, you deal an extra `dice: 1d6` Force damage. Each time the weapon gains this property after the first, the extra damage increases by `dice: 1d6`, to a maximum of `dice: 3d6`. Reroll if the *Moonblade* already deals an extra `dice: 3d6` Force damage on a hit. |
...
| 81-85 | The *Moonblade* scores a [Critical Hit](critical-hit.md) on a roll of 19 or 20 on the `dice: d20`. |
| 86-95 | You can take a [Bonus Action](bonus-action.md) to cause the *Moonblade* to flash brightly. Each other creature that is within 30 feet of you and not behind [Total Cover](Total Cover) must succeed on a DC 15 Constitution saving throw or have the [Blinded](Conditions.md#Blinded) condition for 1 minute. A creature repeats the save at the end of each of its turns, ending the effect on itself on a success. You can't use this property again until you finish a [Short](short-rest.md) or [Long Rest](long-rest.md). |
| 96-99 | The Moonblade has the properties of a [Ring of Spell Storing](Ring-of-Spell-Storing.md). |
| 100 | You can take a [Magic](Magic.md) action to conjure a spectral entity that resembles a shadowy elf if you don't already have one serving you. The entity appears in an unoccupied space within 120 feet of you. It uses the [Shadow](Shadow.md) stat block with these changes: it is a Fey, has a Neutral alignment, and doesn't create new shadows. You control this entity, deciding how it acts and moves. It remains until it drops to 0 [Hit Points](hit-points.md) or you dismiss it as a [Magic](Magic.md) action. |
^moonblade-properties
