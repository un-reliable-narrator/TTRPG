---
obsidianUIMode: preview
cssclasses:
- json5e-item
tags:
- src/5e/dmg
- item/rarity/varies
aliases:
- "Horn of Valhalla"
---
# Horn of Valhalla
*Wondrous item*  
![](items/XDMG/Horn of Valhalla.webp#right)


You can take a [Magic](Magic.md) action to blow this horn. In response, warrior spirits from the plane of Ysgard appear in unoccupied spaces within 60 feet of you. Each spirit uses the [Berserker](Berserker.md) stat block and returns to Ysgard after 1 hour or when it drops to 0 [Hit Points](hit-points.md). The spirits look like living, breathing warriors, and they have [Immunity](Immunity.md) to the [Charmed](Conditions.md#Charmed) and [Frightened](Conditions.md#Frightened) conditions. Once you use the horn, it can't be used again until 7 days have passed.

Four types of Horn of Valhalla are known to exist, each made of a different metal. The horn's type determines how many spirits it summons, as well as the requirement for its use. The DM chooses the horn's type or determines it randomly by rolling on the following table. If you blow the horn without meeting its requirement, the summoned spirits attack you.

If you meet the requirement, they are [Friendly](friendly-attitude.md) to you and your allies and follow your commands.

| dice: 1d100 | Horn Type | Spirits | Requirement |
|-------------|-----------|---------|-------------|
| 01-40 | [Silver](Horn-of-Valhalla-Silver.md) | 2 | None |
| 41-75 | [Brass](horn-of-Valhalla-Brass.md) | 3 | [Proficiency](proficiency.md) with all Simple weapons |
| 76-90 | [Bronze](Horn-of-Valhalla-Bronze.md) | 4 | Training with all Medium armor |
| 91-00 | [Iron](horn-of-Valhalla-Iron.md) | 5 | [Proficiency](proficiency.md) with all Martial weapons |
^1-horn-type-spirits-requirement

**Items in this group:**

- [Horn of Valhalla, Brass](horn-of-Valhalla-Brass.md)
- [Horn of Valhalla, Bronze](Horn-of-Valhalla-Bronze.md)
- [Horn of Valhalla, Iron](horn-of-Valhalla-Iron.md)
- [Horn of Valhalla, Silver](Horn-of-Valhalla-Silver.md)

*Source: Dungeon Master's Guide (2024) p. 270. Available in the <span title='Systems Reference Document (5.2)'>SRD</span> and the Free Rules (2024)*